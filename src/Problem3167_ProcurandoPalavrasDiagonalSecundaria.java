...
public class Problem3167_ProcurandoPalavrasDiagonalSecundaria {
    static char[][] matriz;
    static int linhas;
    static int colunas;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < linhas; i++) {
            matriz[i] = new StringBuilder(sc.nextLine().trim().toLowerCase(Locale.ROOT)).reverse().toString().toCharArray();
        }
    ...
    static void pesquisarPalavra(String palavra) {
        ...
        if (diagonal.contains(palavra) || diagonal.contains(palavraInvertida)) {
            System.out.printf("1 Palavra \"%s\" na diagonal secundaria\n", palavra);
            return;
        }
        ...
        for (int i = 0; i < colunas - 1; i++) {
            ...
            if (acimaDiagonal[i].contains(palavra) || acimaDiagonal[i].contains(palavraInvertida)) {
                System.out.printf("2 Palavra \"%s\" acima da diagonal secundaria\n", palavra);
                return;
            }
        }
        ...
        for (int i = 0; i < linhas - 1; i++) {
            ...
            if (abaixoDiagonal[i].contains(palavra) || abaixoDiagonal[i].contains(palavraInvertida)) {
                System.out.printf("3 Palavra \"%s\" abaixo da diagonal secundaria\n", palavra);
                return;
            }
        }
